...
export function HomeScreen() {
  ...
  const handleOpenDataShortcut = (e, chatNum) => {
    e.stopPropagation();
    setActiveDataChat(chatNum);
    setCurrentView("data_chat");
  };

  const handleOpenAnalyzeShortcut = (e, chatNum) => {
    e.stopPropagation();
    ...
    <main className="home-container">
      ...
      <div className="home-grid">
        ...
        >
          ...
          {/* 4 CHAT SHORTCUT BUTTONS */}
          <div className="home-shortcuts-box" onClick={e => e.stopPropagation()}>
            <span className="shortcuts-label">Quick Chat Rooms:</span>
            <div className="shortcuts-grid">
              {[1, 2, 3, 4].map(chatNum => {
                ...
                return (
                  <button
                    key={chatNum}
                    className="shortcut-btn data-shortcut"
                    onClick={e => handleOpenDataShortcut(e, chatNum)}
                    title={`Open & Record for ${name}`}
                  >
                    <span className="shortcut-num">{chatNum}</span>
                    <span className="shortcut-title">{name}</span>
                    <span className="shortcut-mic-icon">🎙️</span>
                  </button>
                );
              })}
            </div>
        ...
        >
          ...
          <div className="home-shortcuts-box" onClick={e => e.stopPropagation()}>
            <span className="shortcuts-label">Quick Business Rooms:</span>
            <div className="shortcuts-grid">
              {[1, 2, 3, 4].map(chatNum => {
                ...
                return (
                  <button
                    key={chatNum}
                    className="shortcut-btn analyze-shortcut"
                    onClick={e => handleOpenAnalyzeShortcut(e, chatNum)}
                    title={`Open & Record for ${name}`}
                  >
                    <span className="shortcut-num">{chatNum}</span>
                    <span className="shortcut-title">{name}</span>
      ...
      <div className="home-info-box">
        ...
        <p>
          Tap any quick shortcut <b>1, 2, 3, or 4</b> to record spoken numbers (e.g. <code>245 334 551</code>), verify recorded numbers, and save directly into WhatsApp or WhatsApp Business chat rooms.
        </p>
      </div>
    </main>
  );
